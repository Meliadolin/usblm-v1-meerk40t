...
Reuses the generic choice-driven windows and operation panels from
meerk40t.balormk.gui (Configuration, Controller, operation properties) - they
render whatever choices the V1Device registers. The balormk scene module is
NOT reused: its only user-facing part is the correction-file widget, which is
dead UI on the V1 (the board refuses 0x0015 correction payloads), so the
...
def plugin(service, lifecycle):
    if lifecycle == "service":
        return "provider/device/usblmv1"
    ...
    if lifecycle == "added":
        ...
        from meerk40t.balormk.gui.balorconfig import BalorConfiguration
        from meerk40t.balormk.gui.balorcontroller import BalorController
        from meerk40t.balormk.gui.baloroperationproperties import (
            BalorOperationPanel,
        )
        ...
        from usblm_v1.scene import register_scene

        register_scene(service)

        service.add_service_delegate(V1Gui(service))
